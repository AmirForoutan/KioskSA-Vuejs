...
function exportExcel() {
  ...
  exportToExcel(
    filtered.value.map((row) => ({ ...row, Discount: invoiceDiscount(row), CashPrice: paymentPart(row, "cash"), PosPrice: paymentPart(row, "pos"), CreditPrice: paymentPart(row, "credit"), RefundAmount: refundAmount(row), NetPaidAmount: netPaidAmount(row) })),
    [
...
async function printSalesItemsReport() {
  ...
  try {
    ...
    printReceipt(
      "جمع فروش به همراه اقلام",
      reportRange(from.value, to.value),
      `<div class="section"><div class="section-title">سرجمع</div>${moneyPair("جمع فروش", totalPayable.value)}${moneyPair("جمع تخفیف", totalDiscount.value)}${moneyPair("تعداد فاکتور", filtered.value.length)}</div>
...
<template>
  <div class="r-shell">
    <div class="r-toolbar">
      ...
      <button class="r-btn primary" :disabled="loading" @click="loadReport">{{ loading ? "در حال دریافت" : "اعمال فیلتر" }}</button>
    ...
    <div class="r-summary">
      <div><span>تعداد فاکتور</span><b>{{ filtered.length.toLocaleString() }}</b></div>
      <div><span>جمع قابل پرداخت</span><b>{{ totalPayable.toLocaleString() }}</b></div>
      <div><span>جمع تخفیف</span><b>{{ totalDiscount.toLocaleString() }}</b></div>
      <div><span>نقدی</span><b>{{ totalCash.toLocaleString() }}</b></div>
      <div><span>کارتخوان</span><b>{{ totalPos.toLocaleString() }}</b></div>
      <div><span>اعتباری</span><b>{{ totalCredit.toLocaleString() }}</b></div>
      <div><span>عودت</span><b>{{ totalRefund.toLocaleString() }}</b></div>
      <div><span>خالص تسویه</span><b>{{ totalNetPaid.toLocaleString() }}</b></div>
      <div><span>جمع مالیات</span><b>{{ totalTax.toLocaleString() }}</b></div>
    </div>
    ...
    <div class="r-table">
      <div class="r-tr r-th" :class="{ 'has-actions': canManageInvoices }">
        <div>شماره</div><div>تاریخ</div><div>ساعت</div><div>مشتری</div><div>نوع سفارش</div><div>جمع خام</div><div>تخفیف</div><div>مالیات</div><div>بسته‌بندی</div><div>نقدی</div><div>کارتخوان</div><div>اعتباری</div><div>عودت</div><div>خالص تسویه</div><div>آخرین ویرایش</div><div>قابل پرداخت</div><div v-if="canManageInvoices">عملیات</div>
      </div>
      <div v-if="loading" class="r-empty">در حال بارگذاری...</div>
      <div class="r-tr" :class="{ 'has-actions': canManageInvoices }" v-for="row in filtered" :key="row.SaleInvoiceId" @contextmenu="openContextMenu($event, row)">
        <div class="bold">{{ row.SaleInvoiceNumberDay }}</div><div>{{ row.OrderDate }}</div><div>{{ row.OrderTime }}</div>
        <div class="customer-cell"><span class="bold">{{ row.CustomerName || "بدون مشتری" }}</span><small v-if="row.Phone"> - {{ row.Phone }}</small></div>
        <div>{{ row.InvoiceTypeName }}</div><div>{{ money(row.Price).toLocaleString() }}</div><div class="pay discount">{{ invoiceDiscount(row).toLocaleString() }}</div><div>{{ money(row.Tax).toLocaleString() }}</div><div>{{ money(row.PackingPrice).toLocaleString() }}</div><div class="pay cash">{{ paymentPart(row, "cash").toLocaleString() }}</div><div class="pay pos">{{ paymentPart(row, "pos").toLocaleString() }}</div><div class="pay credit">{{ paymentPart(row, "credit").toLocaleString() }}</div><div class="pay refund">{{ refundAmount(row).toLocaleString() }}</div><div class="pay net">{{ netPaidAmount(row).toLocaleString() }}</div><div>{{ row.LastModifyDate }}</div><div class="bold">{{ money(row.Payable).toLocaleString() }}</div>
        <div v-if="canManageInvoices" class="row-actions"><button v-if="canEditInvoices" class="r-mini" :disabled="editingInvoiceId === row.SaleInvoiceId" @click="startEdit(row)">{{ editingInvoiceId === row.SaleInvoiceId ? "..." : "ویرایش" }}</button><button v-if="canDeleteInvoices" class="r-mini danger" :disabled="deletingInvoiceId === row.SaleInvoiceId" @click="deleteInvoice(row)">{{ deletingInvoiceId === row.SaleInvoiceId ? "..." : "حذف" }}</button></div>
      </div>
    </div>
...
.r-toolbar { display: grid; grid-template-columns: minmax(220px,1.2fr) minmax(160px,1fr) minmax(160px,1fr) auto auto auto auto; gap: 10px; align-items: center; }
.r-input, .r-btn { min-height: 46px; border-radius: 8px; padding: 9px 11px; font-size: 15px; color: #eef2ff; background: rgba(255,255,255,.04); border: 1px solid rgba(255,255,255,.1); }
.r-btn { cursor: pointer; white-space: nowrap; } .r-btn.primary { font-weight: 900; background: rgba(20,184,166,.18); border-color: rgba(20,184,166,.34); } .r-btn:disabled { cursor: not-allowed; opacity: .55; }
.r-summary { display: grid; grid-template-columns: repeat(9, minmax(130px,1fr)); gap: 10px; }
.r-summary div, .r-message, .r-empty { border-radius: 8px; padding: 10px 12px; background: rgba(255,255,255,.035); border: 1px solid rgba(255,255,255,.08); }
.r-summary span { color: #a7b0c3; margin-left: 8px; } .r-message { color: #fde68a; background: rgba(245,158,11,.12); border-color: rgba(245,158,11,.22); }
.r-table { flex: 1; min-height: 0; overflow: auto; border-radius: 8px; border: 1px solid rgba(255,255,255,.08); background: rgba(255,255,255,.02); }
.r-tr { display: grid; grid-template-columns: 80px 108px 88px minmax(190px,1.4fr) 110px 110px 100px 100px 100px 100px 110px 100px 100px 120px 120px 130px; min-width: 1880px; gap: 10px; align-items: center; padding: 11px 12px; border-bottom: 1px solid rgba(255,255,255,.06); }
.r-tr.has-actions { grid-template-columns: 80px 108px 88px minmax(190px,1.4fr) 110px 110px 100px 100px 100px 100px 110px 100px 100px 120px 120px 130px 130px; min-width: 2020px; }
.r-tr>div { min-width: 150px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; } .r-tr>.row-actions { overflow: visible; }
.r-th { font-weight: 900; color: #a7b0c3; background: rgba(16,19,26,.96); position: sticky; top: 0; z-index: 2; }
.bold { font-weight: 800; } small, .customer-cell small { color: #a7b0c3; } .pay { font-weight: 900; } .pay.cash { color: #fde68a; } .pay.pos { color: #bfdbfe; } .pay.discount { color: #fca5a5; } .pay.credit { color: #ccfbf1; } .pay.refund { color: #fecaca; } .pay.net { color: #bbf7d0; }
.row-actions { display: flex; gap: 6px; } .r-mini { min-height: 34px; border-radius: 8px; padding: 6px 9px; color: #eef2ff; background: rgba(255,255,255,.04); border: 1px solid rgba(255,255,255,.1); cursor: pointer; } .r-mini.danger { color: #fecaca; background: rgba(239,68,68,.1); border-color: rgba(239,68,68,.22); } .r-mini:disabled { cursor: not-allowed; opacity: .55; }
...
@media (max-width:1180px) { .r-toolbar { grid-template-columns: repeat(2,minmax(0,1fr)); } .r-summary { grid-template-columns: repeat(3,minmax(0,1fr)); } }
</style>
