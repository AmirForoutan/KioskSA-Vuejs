...
function paymentPart(row: DesktopInvoice, key: "pos" | "cash" | "credit") {
  ...
  if (pos + cash + credit === 0 && money(row.Payable) > 0) return key === "pos" ? money(row.Payable) : 0;
...
function groupByCustomer(invoices: DesktopInvoice[]) {
  ...
  invoices.forEach((invoice) => {
    ...
    if ((!current.customer || current.customer === "بدون مشتری") && invoice.CustomerName) current.customer = invoice.CustomerName;
