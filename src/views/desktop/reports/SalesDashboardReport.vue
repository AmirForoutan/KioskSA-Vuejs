...
function renderCharts() {
  ...
  dailyChart = createChart(dailyCanvas.value, {
    ...
    data: { labels: daily.map((row) => row.title), datasets: [
      { type: "bar" as const, label: "فروش روزانه", data: daily.map((row) => row.total), backgroundColor: daily.map((_, i) => chartSoftColors[i % chartSoftColors.length]), borderColor: daily.map((_, i) => chartColors[i % chartColors.length]), borderWidth: 1, borderRadius: 9, maxBarThickness: 38 },
      { type: "line" as const, label: "میانگین متحرک", data: movingAverage(daily.map((row) => row.total)), borderColor: "#fef3c7", backgroundColor: "rgba(254,243,199,.14)", pointBackgroundColor: "#f59e0b", pointRadius: 3, tension: .35, fill: true },
    ] },
...
<template>
  <div class="dash-shell">
    ...
    <div class="dash-grid">
      <section class="dash-panel wide hero-panel">
        <div class="panel-title"><span>روند فروش روزانه</span><small>۱۴ روز آخر + میانگین متحرک</small></div>
        <div class="chart-box"><canvas ref="dailyCanvas"></canvas></div>
      </section>

      <section class="dash-panel">
        <div class="panel-title"><span>سهم نوع سفارش</span><small>سالن / بیرون‌بر</small></div>
        <div class="chart-box"><canvas ref="typeCanvas"></canvas></div>
      </section>

      <section class="dash-panel">
        <div class="panel-title"><span>ترکیب پرداخت</span><small>نقدی، کارتخوان، اعتباری</small></div>
        <div class="chart-box"><canvas ref="paymentCanvas"></canvas></div>
        <div class="payment-breakdown"><div v-for="(row, index) in paymentBreakdown" :key="row.key"><i :style="{ background: chartColors[index] }"></i><span>{{ row.title }}</span><b>{{ row.total.toLocaleString() }}</b></div></div>
      </section>

      <section class="dash-panel wide">
        <div class="panel-title"><span>فروش و تخفیف روزانه</span><small>مقایسه مبلغ فروش با تخفیف</small></div>
        <div class="chart-box"><canvas ref="discountCanvas"></canvas></div>
      </section>

      <section class="dash-panel">
        <div class="panel-title"><span>فروش ساعتی</span><small>شناسایی ساعت‌های شلوغ</small></div>
        <div class="chart-box"><canvas ref="hourlyCanvas"></canvas></div>
      </section>

      <section class="dash-panel">
        <div class="panel-title"><span>مشتریان برتر</span><small>براساس جمع فروش</small></div>
        <div class="chart-box"><canvas ref="customerCanvas"></canvas></div>
        <div class="rank-list"><div v-for="(row, index) in topCustomers" :key="row.key" class="rank-row"><i :style="{ background: chartColors[index % chartColors.length] }"></i><span>{{ row.title }}</span><b>{{ row.total.toLocaleString() }}</b></div></div>
      </section>
    </div>
  </div>
</template>
...
.dash-btn { cursor: pointer; white-space: nowrap; } .dash-btn.primary { font-weight: 900; background: linear-gradient(135deg, rgba(20,184,166,.3), rgba(59,130,246,.18)); border-color: rgba(20,184,166,.42); } .dash-btn:disabled { opacity: .55; cursor: not-allowed; }
