...
function printCreditReceiptsReport() {
  ...
  printReceipt(
    ...
    `<div class="section"><div class="section-title">سرجمع رسیدها</div>${moneyPair("افزایش اعتبار", increaseTotal.value)}${moneyPair("مصرف اعتبار", creditUseTotal.value)}${moneyPair("پرداخت نقدی", cashReceiptTotal.value)}${moneyPair("عودت", refundTotal.value)}${moneyPair("تعداد رسید", filtered.value.length)}</div><div class="section"><div class="section-title">رسیدها</div><table><thead><tr><th>#</th><th>نوع</th><th>مشتری</th><th class="num">مبلغ</th></tr></thead><tbody>${rowsHtml}</tbody></table></div>`
  );
}
</script>
...
<template>
  <div class="cr-shell">
    <div class="cr-toolbar">
      ...
      <button class="cr-btn primary" :disabled="loading" @click="loadReport">{{ loading ? "در حال دریافت" : "اعمال فیلتر" }}</button>
...
.cr-toolbar { display: grid; grid-template-columns: minmax(220px,1.2fr) minmax(130px,.7fr) minmax(130px,.7fr) auto auto auto; gap: 10px; }
.cr-input, .cr-btn { min-height: 46px; border-radius: 8px; padding: 9px 11px; color: #eef2ff; background: rgba(255,255,255,.04); border: 1px solid rgba(255,255,255,.1); }
.cr-btn { cursor: pointer; white-space: nowrap; } .cr-btn.primary { font-weight: 900; background: rgba(20,184,166,.18); border-color: rgba(20,184,166,.34); } .cr-btn:disabled { cursor: not-allowed; opacity: .55; }
...
@media (max-width:1180px) { .cr-toolbar, .cr-summary { grid-template-columns: repeat(2,minmax(0,1fr)); } }
</style>
